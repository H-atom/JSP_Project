...
<head>
    ...
    <script type="text/javascript">
        ...
        window.onload=function(){
            ...
            courseChoose();
            <%
            String strings=(String) request.getAttribute("input");
            if (strings.isEmpty()){%>
            shows(document.getElementById("bottoms"));
            <%}else{%>
            shows(document.getElementById("bottom1"));
            <%}%>
                ...
                <ul style="margin: 0;padding: 0">
                    <li style="margin: 0;height: 280px;background-color: #f3f5f8;border: 1px solid darkgray;overflow: auto">
                        <table class="chooseTable" border="1" style="margin: 0">
                            ...
                            <tbody>
                            <table class="bottomTable" border="=1" id="bottoms">
                                <form>
                                    <%
                                        String stringMajor=(String) request.getSession().getAttribute("major");
                                        showTimetable showTimetable1=new showTimetable();
                                        ArrayList<User> arrayList1=showTimetable1.shows(strings,stringMajor);
                                    ...
                                    <tr>
                                        ...
                                        <td><%=arrayList1.get(i).getTeacher()%></td>
                                        <td><%=arrayList1.get(i).getCredit()%></td>
                                        <td><button style='margin:5px;height:30px;width:50px;text-align:center' type='submit' formmethod='post' onclick='change(this,document.querySelectorAll("#bottoms input"),<%=i%>)'>选课</button></td>
                                    </tr>
                                    <%}%>
                                </form>
                            </table>
                            <table class="bottomTable" border="=1" id="bottom1">
                                <form>
                                    <%
                                        String string=(String) request.getAttribute("name");
                                        String string1=(String) request.getAttribute("number");
                                        String string2=(String) request.getAttribute("teacher");
                                        showTimetable showTimetable2=new showTimetable();
                                        ArrayList<User> arrayList3=showTimetable2.showD(string,string1,string2,stringMajor);
                                        for (int i = 0; i < arrayList3.size(); i++) {%>
                                    <tr>
                                        <td><%=arrayList3.get(i).getTerm()%></td>
                                        <td><%=arrayList3.get(i).getCourseNumber()%> <input type="hidden" value="<%=arrayList3.get(i).getCourseNumber()%>"/></td>
                                        <td><%=arrayList3.get(i).getCourseName()%></td>
                                        <td><%=arrayList3.get(i).getTypes()%></td>
                                        <td><%=arrayList3.get(i).getTeacher()%></td>
                                        <td><%=arrayList3.get(i).getCredit()%></td>
                                        <td><button style='margin:5px;height:30px;width:50px;text-align:center' type='submit' formmethod='post' onclick='change(this,document.querySelectorAll("#bottom1 input"),<%=i%>)'>选课</button></td>
                                    </tr>
                                    <%}%>
                                </form>
                            </table>
